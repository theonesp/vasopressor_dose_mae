...
First_HCT<-First_HCT%>%
  select(Case_Name,result_name,result_value)
...
IO_data_Blood_summarized<-IO_data_Blood_summarized%>%
  select(Case_Name,`Cell Saver`,Blood)
...
IO_data_Fluids_summarized<-IO_data_Fluids_summarized%>%
  select(Case_Name,Colloids_ml,Crystalloids_ml)
...
IO_data_Urine_Output_summarized<-IO_data_Urine_Output_filtered%>%
  select(Case_Name,Urine_Output,VALUE)%>%
